...
public class ExportBookmarksTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
        path = BrowserUtility.exportBookmarks(context);
    ...
    protected void onPostExecute(Boolean result) {
        ...
        if (result) {
            jmmToast.show(context, context.getString(R.string.toast_export_bookmarks_successful) + path);
        } else {
            jmmToast.show(context, R.string.toast_export_bookmarks_failed);
        }
    }
}
